...
class Swagger {
	...
	merge(swagger, options = {}) {
		...
		if (!mergeBlacklist.includes('security')
			&& mergeBlacklist.includes('securityDefinitions')) {
	 ...
	 * @param {object} swagger - A well defined swagger document.
	 * @param {object} [options] - Options for controlling the merge.
	 * @param {object} [options.defaultResponses] - Applies a default set of responses
	 *  to all methods when merging, but will not override existing responses.
	 * @param {object} [options.responses] - Applies all responses to all
	 * methods when merging, overriding any existing responses.
	 * @param {string} [options.prefix] - All paths will be prefixed with `prefix`.
	 * @param {boolean} [options.express = true] - All expressjs paths will be rewritten to Swagger.
	 * @param {pathFilter} [options.filter] - Selectively filter paths.
	...
	_ensureValidParameter(param) {
		...
		if (param.type) {
			...
			const types = [ 'string', 'number', 'integer', 'boolean', 'array', 'file' ];
			if (types.indexOf(param.type) < 0) {
