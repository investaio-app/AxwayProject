...
/**
 * Gets an array of IP filters capable of testing IPv4 and IPv6 addresses and
 * ranges.
 *
 * @param {string[]} allowedIps - An array of IP addresses to allow.
 * @return {object[]} An array of IP filters, e.g.
 * `filters.find(a => a.match(ip))`
 */
function getIPFilters(allowedIps = []) {
	...
	return allowedIps.map((addr) => {
		if (ipaddr.IPv4.isValid(addr)) {
			// If the IP to test is IPv4, then it needs to be mapped to IPv6
			// equivalent. The filter will conver the ip and test against
			// `addr`.
			return {
				match: (ip) => {
					return ipaddr.parse(addr)
						.toIPv4MappedAddress().match(ip, 128);
				}
			};
		} else if (ipaddr.IPv6.isValid(addr)) {
			...
			// If the address contains a slash, then it is a range.  Parse the
			// CIDR range to get the network segment bits.
			...
			return {
				match: (ip) => {
					if (segment.kind() === 'ipv4'
						...
						&& ip.isIPv4MappedAddress()) {
						// The allowed range is IPv4, and the IP address is
						// also IPv4 (as IPv6), so convert IP back to IPv4 and
						// test range.
						const ipv4 = ip.toIPv4Address();
						return ipv4.match(segment, bits);
					} else if (segment.kind() === 'ipv6') {
						// The allowed range is IPv6, and the IP address is
						// IPv6, safe to match
 ...
 * @param {string[]} [allowedIps] - An array of IPv4 or IPv6 addresses.  If
 * not provided or empty, no filtering is done. return {function} Express
 * middleware function for IP filtering
 * @return {function} - A middleware function for IP filtering
 */
		...
		if (found) {
			...
			if (options.log) {
				...
				if (addr.isIPv4MappedAddress && addr.isIPv4MappedAddress()) {
					// if IPv6 and is a mapped IPv4 address, then convert it to
					// IPv4 so that it looks normal in the log.
