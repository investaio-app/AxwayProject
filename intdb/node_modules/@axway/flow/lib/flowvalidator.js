...
class FlowValidator extends FlowGraph.Validator {
	 ...
	 *  code: MISSING_VALUE, NOT_IN_SPEC, INVALID_TYPE, INVALID_VALUE,
	 *        INVALID_JSONPATH, INVALID_STRING, INVALID_BOOLEAN,
	 *        INVALID_NUMBER, INVALID_ARRAY, INVALID_NULL, INVALID_OBJECT
	 *
	...
	/**
	 * Checks validation for nodes and adds invalid parameters to validatationData
	 * Possible errors and error codes:
	 *  type: invalidNodeParameter
	 *  code: MISSING_VALUE, NOT_IN_SPEC, INVALID_TYPE, INVALID_VALUE,
	 *        INVALID_JSONPATH, INVALID_STRING, INVALID_BOOLEAN,
	 ...
	 *        UNKNOWN_CREDENTIAL
	 * @private
	 * @param {string} location type of parameter ('parameters' or 'authorizations')
	 * @param {string} nodeId id of the node to check parameters for
	 * @param {object} validationData validation data object
	 * @return {boolean} `true` if invalid.
	 */
	_checkNodeParam(location, nodeId, validationData) {
		...
		const {
			...
			parameters,
			methodExists
		} = this._getNodeDetailsById(nodeId, location);
		const schemaTypeMap = {
			string: 'string',
			boolean: 'boolean',
			integer: 'number',
		...
		parameters.forEach((param) => {
			let specParam = specParams
				&& specParams.properties
				&& specParams.properties[param.name];
			...
			if (specParam
				...
				// it's not going to be jsonpath but it's an expected type.
				expectedTypes.push('jsonpath');
	 ...
	 *
	 *  type: invalidNodeOutput
	 *  code: INVALID_JSONPATH, MISSING_KEY, NOT_IN_SPEC
	 *
	 *  type: deprecatedNodeHandler
	 *  code: DEPRECATED
