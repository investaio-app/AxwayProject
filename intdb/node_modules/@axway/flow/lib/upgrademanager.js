...
class UpgradeManager {
	 ...
	 * @param {function} accessors.getVersion - function which returns the version of the data
	 *		passed
	...
	registerUpgradeType(type, accessors, validator) {
		...
		[ 'getVersion', 'setVersion' ].forEach(key => {
	...
	_getUpgrade(type) {
		return async (data, id) => {
			const { getVersion } = this.types[type].accessors;
	...
	_upgradeVersion(type, data, id) {
		...
		this.logger
			.debug(`Upgrading ${type} ${id} from version ${version} to version ${update.to}`);
		// run all upgrades
		return promiseForEach([
			...update.pre,
			update.main,
			...update.post
		], (upgrade, upgradedData) => {
	...
	_runUpgrade(data, upgrade) {
		...
		this.logger
			.debug(`Running ${type} ${stage} upgrade step from version ${from} to version ${to} ([${owner}] ${id})`);
