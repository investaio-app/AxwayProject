...
const { formatNodeName } = require('./utils');
...
class Flow extends FlowGraph {
	 ...
	 * @param {object} [options.logger] - logger to use in the flow.
	 * @param {object} [options.credentialManager] - API Builder credential manager.EventEmitter
	 * @param {boolean} [options.wasUpgraded] - True if the flow was upgraded on load.
	 */
	constructor(flowId, flowdef, options) {
		...
		const { logger, credentialManager, wasUpgraded } = options;
		this.wasUpgraded = wasUpgraded || false;
	 ...
	 * @param {object} [options.validateFlowParameter=true] - Validates `data` against flow input
	 * parameter.
	...
	_invokeNodes(ctx, nodes) {
		...
		const isTrace = ctx.runtime.logger.willLogAt('TRACE');
		const isDebug = ctx.runtime.logger.willLogAt('DEBUG');
			...
			this._invokeWhenReady(ctx, node)
				.then(
					...
					(err) => {
						if (isTrace) {
							ctx.runtime.logger
								.trace(`${this._getNodeDisplayName(node)} rejected: ${err}`);
						}
						throw err;
					})
				...
				.catch((err) => {
					if (isDebug) {
						ctx.runtime.logger.debug(
							chalk.dim('Aborting flow:', err), err.stack);
					}
	...
	/**
	 * Recursively prunes and resolves all nodes which occur previous in the flow to a list of
	 * nodes.
	 *
	 * @private
	...
	_resolveParentNodePaths(ctx, nodes) {
		...
		nodes.forEach((node) => {
			const nodeName = this._getNodeDisplayName(node);

			if (isTrace) {
				ctx.runtime.logger.trace(' ',
					chalk.green(nodeName),
					chalk.dim('pruning node parents')
				);
			}
			...
			parents.forEach((parent) => {
				const parentName = this._getNodeDisplayName(parent);
				if (isTrace) {
					ctx.runtime.logger.trace(' ',
						chalk.green(nodeName),
						chalk.dim(`found parent ${chalk.cyan(parentName)}`)
					);
				}
				// if there isn't a path to the parent then return. (could have already been
				// resolved)
				if (!reversePaths[node][parent]) {
					if (isTrace) {
						ctx.runtime.logger.trace(' ',
							chalk.green(nodeName),
							chalk.dim('no route back to'),
							chalk.cyan(parentName)
						);
					}
					return;
				}
				...
				if (isTrace) {
					ctx.runtime.logger.trace(' ',
						chalk.green(nodeName),
						chalk.dim(`parent ${chalk.cyan(parentName)}`),
						chalk.dim(`has ${children.length} child nodes`)
					);
				}
				children.forEach((child) => {
					const childName = this._getNodeDisplayName(child);
					if (child === node) {
						if (isTrace) {
							ctx.runtime.logger.trace(' ',
								chalk.green(nodeName),
								chalk.dim(`parent ${chalk.cyan(parentName)}`),
								chalk.dim(`skipping child ${chalk.yellow(childName)} since it's the node that we're resolving parents for`)
							);
						}
						return;
					}
					...
					if (isTrace) {
						ctx.runtime.logger.trace(' ',
							chalk.green(nodeName),
							chalk.dim(`parent ${chalk.cyan(parentName)}`),
							chalk.dim(`removing route back to
								${chalk.yellow(childName)}`)
						);
					}
					...
					if (!reversePaths[child] || !Object.keys(reversePaths[child]).length) {
						...
						if (isTrace) {
							ctx.runtime.logger.trace(' ',
								chalk.green(nodeName),
								chalk.dim(`cutting inactive paths from
									${chalk.yellow(childName)}`)
							);
						}
						...
						if (isDebug) {
							ctx.runtime.logger.debug(' ',
								chalk.green(nodeName),
								chalk.dim(`Resolving ${chalk.yellow(childName)}`)
							);
						}
				...
				if (!reversePaths[parent] || !Object.keys(reversePaths[parent]).length) {
					if (isDebug) {
						ctx.runtime.logger.debug(' ',
							chalk.green(nodeName),
							chalk.dim(`Resolving ${chalk.cyan(parentName)}`)
						);
					}
				...
				if (isTrace) {
					ctx.runtime.logger.trace(' ',
						chalk.green(nodeName),
						chalk.dim(`removing route to ${chalk.cyan(parentName)}`)
					);
				}
	...
	_invokeWhenReady(ctx, node) {
		...
		const isDebug = ctx.runtime.logger.willLogAt('DEBUG');
		if (isDebug) {
			ctx.runtime.logger.debug(chalk.dim('Waiting:', nodeName));
		}
		const nodePromise = new Promise((resolve) => {
			Promise.all(this._getNodeParentExecutionPromises(ctx, node.id)).then(() => {
				if (isDebug) {
					ctx.runtime.logger.debug(chalk.dim('Invoking:',
						`${nodeName}, method: ${node.node.metaMethod}`));
				}
	...
	_updateContext(responseCtx) {
		...
		if (target && responseCtx.result !== undefined) {
			...
			if (responseCtx.ctx.runtime.logger.willLogAt('DEBUG')) {
				responseCtx.ctx.runtime.logger.debug(
					chalk.green(' ', this._getNodeDisplayName(responseCtx.node)),
					chalk.dim('writing'),
	...
	_selectOutput(respCtx) {
		...
		if (respCtx.ctx.runtime.logger.willLogAt('DEBUG')) {
			respCtx.ctx.runtime.logger.debug(' ',
				chalk.green(this._getNodeDisplayName(respCtx.node)),
				'finished flow-node route:', `${chalk.dim(JSON.stringify(respCtx.next))},`,
				'time:', `${chalk.dim(`${respCtx.time} ms`)}`);
		}
		return respCtx;
	}
	...
	_cutInactivePath(ctx, inactivePaths) {
		...
		inactivePaths.forEach((path) => {
			// if the target hasn't already been resolved, and the target still has reverse link
			// to source
