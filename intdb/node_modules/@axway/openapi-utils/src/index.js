 ...
 * It is facade to the currently used underlying parsers like ajv, swagger-parser, swagger-tools
 * etc.
 *
 ...
 * options.validateRefs - set it to false if references validation and dereferencing must be
 * skipped
...
module.exports = (options = {}) => {
	return {
		 ...
		 */
		validateSwagger: async (swagger, { logger }) => {

			let swaggerParserOutput = JSON.parse(JSON.stringify(swagger));
			// Step 1. Validate with AJV (this happens internally within axway-schema)
			const schemaErrors = validate.againstSchema(swaggerParserOutput);
			if (schemaErrors) {
			...
			try {
				swaggerParserOutput = await validate.withSwaggerParser(
					swaggerParserOutput, options.swaggerParser);
			} catch (err) {
				// Don't throw for operantionId error for backward compatibility, instead warn only
				if (err.message && err.message
					.includes('Validation failed. Duplicate operation id')) {
			...
			// b. We do not want to pass references ($ref) to semantic validation algorithm -
			// preserve previous behvaiour of Swagger.
			//
			// For case b. we find and remove all the references so they are not checked by
			// swagger-tools on semantic validation. Note this might change in major version for
			// OpenAPI Client plugin! Leaving previous strategy commented:
			// const file = (options.validateRefs === false)
			// ? stripReferences(swaggerParserOutput)
			// : dereference(swaggerParserOutput, options.findRefsConfig);

			// We are skipping swagger tools for OpenAPI Client plugin completely until we find
			// better solution. See: https://techweb.axway.com/jira/browse/RDPP-5485
...
/**
 * Dereference $ref occurrences. Note that dereferencing is made with default options specified
 *
