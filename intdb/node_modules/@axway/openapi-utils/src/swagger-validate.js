...
const stSpec20 = require('@axway/swagger-tools/lib/specs').v2_0;
const swagger20 = require('@axway/swagger-tools/schemas/2.0/schema.json');
...
module.exports = {
	...
	/**
	 * Validates using AJV (https://github.com/epoberezkin/ajv) under the hood to check the
	 * swagger against Swagger 2.0 JSON Schema
	 *
	 ...
	 * Warning: `swaggerParser.validate` function modifies the swagger input
	 * by dereferencing all the $ref.
	 *
	 * @param {object} swagger the swagger to be validated
	...
	withSwaggerParser: (swagger, options = {}) => {
		 ...
		 */
		// if we have options merge them over the default ones
		const config = {
			...options,
			resolve: {
				external: false,
		...
		return swaggerParser.validate(swagger, config);
	},
	 ...
	 * Use it in your initial validation to prevent cases where your swagger looks ok but fails
	 * semantic validation later in swagger middleware.
	 *
	 * @param {object} swagger the swagger to be validated
