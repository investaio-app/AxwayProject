...
function initCredential(config, credential) {
	...
	if (tusername === 'string' && tpassword === 'string') {
		...
		credential.token = Buffer
			.from(`${credential.username || ''}:${credential.password || ''}`)
			.toString('base64');
		...
		// both types are non-null or empty but at least one is null. if either username or
		// password is null then credential is `none` and the action is `unauthorized`.
