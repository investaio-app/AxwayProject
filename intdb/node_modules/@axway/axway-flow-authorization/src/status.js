...
/**
 * A list of actions that would default credential's action status to 'warning'(needs user
 * interaction).
 */
 ...
 * - status of the configuration (For the current implementation invalid config fails to start so
 *   this isn't a concern).
 * - status of the credential.
 * - status of lifecycle actions.
 *
 * For example it's possible to have a valid credential whose refresh has errored. The credential
 * is still valid and usable. Hopefully refresh will succeed prior to expiry of the credential.
 *
 * A chart of what it means:
 * | Credential | Action        | Description |
 * |------------|---------------|-------------|
 * | none       | needsAuth     | There is no credential and no way to get one. Requires user
 *                                authorization. |
 ...
 * | none       | needsClient   | There is no credential and the client_id and/or client_secret are
 *                                null |
 * | none       | authError     | There is no credential and was an error in the user
 *                                authorization. |
 * | none       | unauthorized  | There is no credential and user didn't authorize it. |
 * | none       | needsRefresh  | There is no credential but there is sufficient config to
 *                                bootstrap it. |
 * | none       | refreshError  | There is no credential and bootstrapping failed. |
 * | expired    | needsAuth     | The credential is expired and no way to refresh. Requires user
 *                                authorization. |
 * | expired    | authError     | The credential is expired and was an error in the user
 *                                authorization. |
 * | expired    | unauthorized  | The credential is expired and user didn't authorize it. |
 * | expired    | needsRefresh  | The credential is expired but there is sufficient config to
 *                                refresh it. |
 * | expired    | refreshError  | The credential is expired and refresh failed. |
 * | ok         | refreshError  | The credential is valid but the background refresh of it
 *                                failed. |
 * | ok         | none          | The credential is valid and no action required. |
 *
