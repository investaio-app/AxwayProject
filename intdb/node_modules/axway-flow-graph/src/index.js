...
const { validateJsonPathSelector } = require('./jsonpath');
...
class FlowGraph {
	...
	changeMethod(nodeId, method, nodehandler) {
		...
		Object.keys(source.outputs).forEach(outputName => {
			if (!specOutputs[outputName]) {
				// delete the output (and all it's routes / edges) if it doesn't exit in the new
				// spec
		...
		const nodeMethodName = (source.method && nodehandler.methods[source.method]
	...
	deleteRoute(output, fromId, toId) {
		...
		if (node.outputs && node.outputs[output]) {
			if (node.outputs[output].routes && node.outputs[output].routes.length) {
				node.outputs[output].routes
					= node.outputs[output].routes.filter(route => route !== toId);
			}
		}
	}
	...
	deleteRoutes(fromId, toId) {
		...
		if (node.outputs && Object.keys(node.outputs).length > 0) {
			Object.keys(node.outputs).forEach(output => {
				if (node.outputs[output].routes && node.outputs[output].routes.length) {
					node.outputs[output].routes = node.outputs[output].routes
	...
	_updateParameter(location, nodeId, name, prop, newValue) {
		...
		this._addParameter(
			location, nodeId, newParameter.name, newParameter.value, newParameter.type);
	}
	 ...
	 */
	getDescription() {
		return this.flow.info.description;
	}
	...
	setDescription(description) {
		this.flow.info.description = description;
		this._onChange();
	}
...
FlowGraph.validateJsonPathSelector = validateJsonPathSelector;
