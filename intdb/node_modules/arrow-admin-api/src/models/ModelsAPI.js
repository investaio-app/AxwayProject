...
class ModelsAPI extends APIProvider {
	static bind(controller) {
		...
		const { logger } = controller.ctx.apibuilder;
		...
		swagger
			...
			.action((req, res) => {
				try {
					const errors = Model.validate(req.body.model);
					if (!errors) {
						...
						logger.error('Failed to validate model', JSON.stringify(errors));
						return APIProvider.handleError(
							new BadRequestError('Invalid model.'), res, 400, 500);
					}
				} catch (err) {
					APIProvider.handleError(err, res, 400, 500);
				}
