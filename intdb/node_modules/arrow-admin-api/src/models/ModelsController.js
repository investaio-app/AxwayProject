...
class ModelsController {
	...
	getModels() {
		return new Promise((resolve, reject) => {
			...
			if (!models) {
				...
				// The APIBuilder.Model has a toJson that's more restrictive that limits the data,
				// so re-compose the model list to avoid the issue.
				const bareModels = {};
	...
	*/
	async createModel(modelDef) {
		...
		await util.mkdirIfNotExistsAsync(this.modelsDir);

		/**
		 * Concurrent writes to a same file properly at a given path
		 *
		 * @param {string} filePath - the file path
		 * @param {object} code - object to stringify and write to file
		 */
		await new Promise((resolve, reject) => {
			util.writeFile(filepath, code, (err) => {
				if (err) {
					return reject(err);
				}
				resolve();
